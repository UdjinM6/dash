...
class RpcMiscTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.restart_node(0, ["-txindex=0", "-blockfilterindex=0", "-merkrangeindex=0", "-peerblockfilters=0"])
        # Without any indices running the RPC returns an empty object
        assert_equal(node.getindexinfo(), {})

        # Restart the node with indices and wait for them to sync
        self.restart_node(0, ["-txindex", "-blockfilterindex", "-merkrangeindex=basic", "-coinstatsindex"])
        ...
        assert_equal(
            node.getindexinfo(),
            {
                ...
                "basic merk range index": values,
                "coinstatsindex": values,
            }
        )
        # Specifying an index by name returns only the status of that index
        for i in {"txindex", "basic block filter index", "basic merk range index", "coinstatsindex"}:
