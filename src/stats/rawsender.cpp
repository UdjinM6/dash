...
RawSender::RawSender(const std::string& host, uint16_t port, std::optional<std::string>& error) noexcept :
    m_host{host},
    m_port{port}
{
    if (host.empty()) {
...
std::string RawSender::ToStringHostPort() const noexcept { return strprintf("%s:%d", m_host, m_port); }
