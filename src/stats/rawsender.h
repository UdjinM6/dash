...
struct RawMessage : public std::vector<uint8_t>
{
    ...
    parent_type& append(const parent_type::iterator& rhs_begin, const parent_type::iterator& rhs_end)
    {
        reserve((rhs_end - rhs_begin) + 1);
        insert(end(), rhs_begin, rhs_end);
        return *this;
    }
};
...
public:
    ...
    std::optional<std::string> Send(const RawMessage& msg) noexcept EXCLUSIVE_LOCKS_REQUIRED(!cs);
    std::optional<std::string> SendDirectly(const RawMessage& msg) noexcept;

    std::string ToStringHostPort() const noexcept;

    void QueueAdd(const RawMessage& msg) noexcept EXCLUSIVE_LOCKS_REQUIRED(!cs);
    void QueueFlush() noexcept EXCLUSIVE_LOCKS_REQUIRED(!cs);
    void QueueFlush(std::deque<RawMessage>& queue) noexcept;
    void QueueThreadMain() noexcept EXCLUSIVE_LOCKS_REQUIRED(!cs);
