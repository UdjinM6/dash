...
private:
    ...
    /** Timer to animate the governance cycle icon while syncing. */
    QTimer* m_timer_governance_sync{nullptr};
    /** Pre-cached governance cycle pixmaps keyed by {ThemedColor, frame}. */
    ...
    /** Regenerate all pre-cached governance cycle pixmaps (e.g. after a theme change). */
    void refreshGovernanceCycleIcons();

    /** Recompute the governance cycle icon state, pixmap and tooltip. */
