...
PeerMsgRet CGovernanceManager::ProcessMessage(CNode& peer, CMasternodeSync& mn_sync, CConnman& connman,
                                              PeerManager& peerman, std::string_view msg_type, CDataStream& vRecv)
{
    if (!IsValid()) return {};
...
void CGovernanceManager::AddGovernanceObject(CGovernanceObject& govobj, CMasternodeSync& mn_sync, PeerManager& peerman,
                                             const CNode* pfrom)
{
    uint256 nHash = govobj.GetHash();
    std::string strHash = nHash.ToString();
...
std::optional<const CGovernanceObject> CGovernanceManager::CreateGovernanceTrigger(
    const std::optional<const CSuperblock>& sb_opt, CMasternodeSync& mn_sync, PeerManager& peerman,
...
void CGovernanceManager::VoteGovernanceTriggers(const std::optional<const CGovernanceObject>& trigger_opt,
                                                const CMasternodeSync& mn_sync, CConnman& connman, PeerManager& peerman,
                                                const CActiveMasternodeManager& mn_activeman)
{
...
bool CGovernanceManager::VoteFundingTrigger(const uint256& nHash, const vote_outcome_enum_t outcome,
                                            const CMasternodeSync& mn_sync, CConnman& connman, PeerManager& peerman,
                                            const CActiveMasternodeManager& mn_activeman)
{
...
bool CGovernanceManager::MasternodeRateCheck(const CMasternodeSync& mn_sync, const CGovernanceObject& govobj,
...
bool CGovernanceManager::MasternodeRateCheck(const CMasternodeSync& mn_sync, const CGovernanceObject& govobj,
                                             bool fUpdateFailStatus, bool fForce, bool& fRateCheckBypassed)
{
    LOCK(cs);
...
void CGovernanceManager::UpdatedBlockTip(const CBlockIndex* pindex, CMasternodeSync& mn_sync, CConnman& connman,
                                         PeerManager& peerman, const CActiveMasternodeManager* const mn_activeman)
{
