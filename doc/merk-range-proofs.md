...
- Max response payload (header + proof): `MAX_MERKRANGE_PROOF_BYTES = 2 MiB`
- If `stop_hash` is not on the active chain, request is ignored

## Runtime Behavior

- If the index is still syncing, the P2P server may serve a prefix of the requested range up to indexed height (instead of dropping the request). The RPC endpoint returns an error in this case.
...
- A change in `snapshot_tip_hash` between sequential `getmerkrange` responses does not invalidate either response and must not by itself cause rejection by a production client. (The functional test asserts snapshot consistency across chunks because it runs in a controlled environment where no new blocks arrive; that assertion is a test-correctness check, not a protocol requirement.)
